...
#
# EdgeSAM is three builds; run this once per config:
#   ./compile.sh --config config_encoder.yaml     --gpu N
#   ./compile.sh --config config_decoder_sp1.yaml --gpu N
#   ./compile.sh --config config_decoder_bp2.yaml --gpu N
#
#   --config : which build (encoder / decoder sp1 / decoder bp2)
...
CONFIG=config_encoder.yaml
GPU=0
while [ $# -gt 0 ]; do
  case "$1" in
    --config) CONFIG=$2; shift 2;;
    --gpu)    GPU=$2;    shift 2;;
    *) echo "unknown arg: $1"; sed -n '2,20p' "$0"; exit 1;;
  esac
done
