...
#
#   --config : which build. config_det.yaml (SCRFD detection),
#              config_rec.yaml (ArcFace int8), config_rec_int16.yaml (ArcFace int16).
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --gpu)    GPU=$2;    shift 2;;
    *) echo "unknown arg: $1"; sed -n '2,17p' "$0"; exit 1;;
  esac
done
