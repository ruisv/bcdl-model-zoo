#!/usr/bin/env python3
"""[2/3] PTQ calibration data for PP-OCRv5 detection, recognition and cls.

Same normalisation story as v6, because v5 uses the same runtime contract.

All three models compile with `input_type_rt: featuremap` and
`norm_type: no_preprocess`: the BPU takes an already-normalised float tensor and
the CPU side does the scaling. So `common/calib_pack.py` cannot enforce the
normalisation from config.yaml (with `no_preprocess` its normalize() is a
pass-through by definition) -- the contract lives between THIS FILE and the
runtime's CPU preprocessing, and the constants below must match what the runtime
feeds. They are PaddleOCR's own preprocessing:

    det : (x/255 - [0.485,0.456,0.406]) / [0.229,0.224,0.225]   (ImageNet)
    rec : (x/255 - 0.5) / 0.5                                   ([-1, 1])
    cls : (x/255 - 0.5) / 0.5                                   ([-1, 1])

Writing still goes through calib_pack.pack() so file naming, dtype and manifest
stay identical to every other model here.
...
NORM = {
    ...
    "cls": (np.array([0.5, 0.5, 0.5], np.float32),
            np.array([0.5, 0.5, 0.5], np.float32)),
}


def preprocess(path: str, task: str, h: int, w: int) -> np.ndarray:
    """Match the DEPLOYMENT preprocessing exactly, per task.

    Calibration must see the same distribution the runtime feeds, and the tasks
    preprocess differently:
      ...
      cls   zeros in normalised space. This is PaddleOCR's resize_norm_img /
            resize_norm_img_cls -- a plain resize to (W,H) stretches a short line
            to fill the width, distorting glyphs AND handing the calibrator a
            distribution the runtime never produces. rec and cls share this path
            (cls just uses 80x160 and decides 0 deg vs 180 deg).

    All use RGB (BGR read, R into channel 0), matching BCDL/ccdl. (Upstream
    ...
    # rec / cls: aspect-preserving + right-pad, mirroring packNchwPad.
...
def main():
    ...
    ap.add_argument("--images", required=True,
                    help="directory or glob. det: page images. rec/cls: line crops.")
    ...
    # look like the pages you will run on, and recognition / cls on REAL line
    # crops (ideally cut by the detector), not on whole pages scaled to H high.
